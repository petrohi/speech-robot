...
int main() {
    ...
    struct tensil_instruction_buffer buffer = {
        .ptr = prog_buffer_ptr, .size = 0x100000, .offset = 0};
    tensil_buffer_reset(&buffer);

    error = tensil_buffer_append_config_instruction(
        &buffer, &layout, TENSIL_CONFIG_REGISTER_DRAM0_OFFSET, 0);
    ...
    error = tensil_buffer_append_program(&buffer, (const u8 *)MODEL_PROG_BASE,
                                         MODEL_PROG_SIZE);
    ...
    while (true) {
        ...
        error = TENSIL_XILINX_RESULT(
            XAxiDma_BdSetLength(cur_bd_ptr, RFFT_TX_FRAME_LINE_SIZE,
        ...
        size_t infer_index =
            prepare_index ? prepare_index - 1 : MODEL_DRAM0_BUFFER_NUMBER - 1;
        ...
        for (size_t i = 0; i < MODEL_DRAM0_BUFFER_NUMBER; i++) {
            ...
            size_t rfft_source_line = shifted_line < 0
                                          ? RFFT_TX_FRAME_HEIGHT + shifted_line
                                          : shifted_line;
            ...
            u8 *rfft_rx_line_ptr =
                rfft_rx_buffer_ptr + rfft_source_line * RFFT_TX_FRAME_LINE_SIZE;
